...
import table from "./calc_table.json" with { type: "json" } // parametri di pred
...
export function calcResults(answers) {
  ...
  const age = Number(responseArray.find(item => item.question === ageQ).Anni)*12 + Number(responseArray.find(item => item.question === ageQ).Mesi)
